<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="list" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="入库类型" prop="orderType" width="100" align="center"><template #default="scope"><dict-tag :options="wms_inbound_type" :value="scope.row.orderType" /></template></el-table-column>
      <el-table-column label="供应商" prop="supplierName" show-overflow-tooltip />
      <el-table-column label="目标仓库" prop="warehouseName" width="120" />
      <el-table-column label="状态" prop="status" width="100" align="center"><template #default="scope"><dict-tag :options="wms_inbound_status" :value="scope.row.status" /></template></el-table-column>
      ...
      <el-table-column label="操作" width="200" align="center">
        <template #default="scope">
          ...
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['wms:inbound:remove']" v-if="scope.row.status === '0'">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="900px" append-to-body>
      <el-form ref="inboundRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-row>
          ...
          <el-col :span="8"><el-form-item label="预计入库" prop="inboundDate"><el-date-picker v-model="form.inboundDate" type="date" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item></el-col>
    ...
    <el-dialog title="入库单详情" v-model="detailOpen" width="900px" append-to-body>
      <el-descriptions :column="3" border>
        ...
        <el-descriptions-item label="预计入库">{{ detailData.inboundDate }}</el-descriptions-item>
...
const data = reactive({ form: {}, queryParams: { pageNum: 1, pageSize: 10, orderNo: undefined, orderType: undefined, status: undefined }, rules: { orderType: [{ required: true, message: '入库类型不能为空', trigger: 'change' }], warehouseId: [{ required: true, message: '目标仓库不能为空', trigger: 'change' }] } })
...
function reset() { form.value = { orderNo: undefined, orderType: '0', supplierId: undefined, warehouseId: undefined, inboundDate: undefined, status: '0', remark: undefined, detailList: [] }; proxy.resetForm('inboundRef') }
...
function handleMaterialChange(index, row) { const material = materialOptions.value.find(m => m.materialId === row.materialId); if (material) { row.isBatchManage = material.isBatchManage; row.isExpiryManage = material.isExpiryManage } }
