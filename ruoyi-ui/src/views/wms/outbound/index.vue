...
function getList() { loading.value = true; listOutbound(proxy.addDateRange(queryParams.value, dateRange.value, 'OutboundDate')).then(res => { list.value = res.rows; total.value = res.total; loading.value = false; applySavedWidths() }) }
...
function handleExport() { proxy.download('wms/outbound/export', { ...proxy.addDateRange(queryParams.value, dateRange.value, 'OutboundDate') }, 'outbound_' + new Date().getTime() + '.xlsx') }
