        ...
        <section class="rd-card">
          ...
          <div class="rd-card-body" v-show="!collapsedCards.vc1" style="display:block;padding:12px 20px">
            <el-table :data="viewData.detailList" border size="small" style="width:100%">
              ...
              <el-table-column label="规格型号" prop="specModel" width="150" show-overflow-tooltip />
              <el-table-column label="单位" prop="unit" width="65" align="center"><template #default="scope"><dict-tag :options="wms_unit" :value="scope.row.unit" /></template></el-table-column>
              <el-table-column label="领料数量" prop="issueQty" width="90" align="center" />
              <el-table-column label="退料数量" prop="returnQty" width="90" align="center" />
              <el-table-column label="批次号" prop="batchNo" width="100" show-overflow-tooltip />
            </el-table>
          </div>
        </section>
        <section class="rd-card">
...
const defaultColumns = { returnNo: { label: '退料编号', visible: true }, workOrderNo: { label: '工单编号', visible: true }, materialCode: { label: '物料编码', visible: true }, materialName: { label: '物料名称', visible: true }, returnQty: { label: '退料数量', visible: true }, returnBy: { label: '退料人', visible: true }, status: { label: '状态', visible: true }, createTime: { label: '创建时间', visible: true } }
...
function getList() { loading.value = true; listReturnMaterial(proxy.addDateRange(queryParams.value, dateRange.value)).then(response => { dataList.value = response.rows; total.value = response.total; loading.value = false; applySavedWidths(); loadStatusCounts(); }).catch(() => { loading.value = false; }); }
