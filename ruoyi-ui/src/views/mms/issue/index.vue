        ...
        <section class="rd-card">
          ...
          <div class="rd-card-body" v-show="!collapsedCards.vc1" style="display:block;padding:12px 20px">
            <el-table :data="viewData.detailList" border size="small" style="width:100%">
              ...
              <el-table-column label="物料名称" prop="materialName" min-width="110" show-overflow-tooltip />
    ...
    <el-dialog v-model="confirmOpen" width="860px" append-to-body draggable class="rd-dialog">
      ...
      <div class="rd-page">
        ...
        <section class="rd-card">
          ...
          <div class="rd-card-body" v-show="!collapsedCards.cc0" style="display:block"><div class="rd-grid"><div class="rd-item"><span class="rd-label">领料编号</span><div class="rd-value">{{ confirmData.issueNo || '-' }}</div></div><div class="rd-item"><span class="rd-label">工单编号</span><div class="rd-value">{{ confirmData.workOrderNo || '-' }}</div></div><div class="rd-item"><span class="rd-label">产品编码</span><div class="rd-value">{{ confirmData.productCode || '-' }}</div></div><div class="rd-item"><span class="rd-label">产品名称</span><div class="rd-value">{{ confirmData.productName || '-' }}</div></div><div class="rd-item"><span class="rd-label">工单数量</span><div class="rd-value">{{ confirmData.planQty != null ? confirmData.planQty : '-' }}</div></div><div class="rd-item"><span class="rd-label">当前状态</span><div class="rd-value"><dict-tag :options="mms_issue_status" :value="confirmData.status" /></div></div></div></div>
        </section>
        <section class="rd-card">
          ...
          <div class="rd-card-body" v-show="!collapsedCards.cc2" style="display:block;padding:16px 20px"><el-form ref="confirmFormRef" :model="confirmData" label-width="100px" size="default"><el-row :gutter="20"><el-col :span="12"><el-form-item label="领料人" prop="issueBy" :rules="[{ required: true, message: '请选择领料人', trigger: 'change' }]"><el-input v-model="confirmData.issueBy" readonly placeholder="请选择领料人" style="width: 100%" @click="openConfirmIssueByPicker"><template #append><el-button icon="Search" @click="openConfirmIssueByPicker" /></template><template #suffix><el-icon v-if="confirmData.issueBy" class="rd-form-tip" style="cursor:pointer" @click.stop="confirmData.issueBy = undefined"><CircleClose /></el-icon></template></el-input></el-form-item></el-col><el-col :span="12"><el-form-item label="领料时间" prop="issueTime" :rules="[{ required: true, message: '请选择领料时间', trigger: 'change' }]"><el-date-picker v-model="confirmData.issueTime" type="datetime" placeholder="选择领料时间" value-format="YYYY-MM-DD HH:mm:ss" style="width: 100%" /></el-form-item></el-col></el-row></el-form></div>
        </section>
        <section class="rd-card">
...
const defaultColumns = { issueNo: { label: '领料编号', visible: true }, workOrderNo: { label: '工单编号', visible: true }, materialCode: { label: '物料编码', visible: true }, materialName: { label: '物料名称', visible: true }, issueQty: { label: '领料数量', visible: true }, issueBy: { label: '领料人', visible: true }, status: { label: '状态', visible: true }, createTime: { label: '创建时间', visible: true } }
...
function formatMultiNames(names) {
  ...
  return arr.slice(0, 2).join('、') + '...等' + arr.length + '种';
}
...
.multi-material-cell{display:inline-flex;align-items:center;gap:6px;max-width:100%}
.multi-material-cell .multi-material-codes{font-size:12px;color:var(--ink-500);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:120px}
.multi-material-cell .multi-material-names{font-size:13px;color:var(--ink-700);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.qty-cell{font-weight:600}
.qty-detail-count{font-size:12px;color:var(--ink-400);margin-left:4px}
