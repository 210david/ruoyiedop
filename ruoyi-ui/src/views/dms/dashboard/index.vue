          ...
          <div class="health-content">
            <div class="health-item">
              ...
              <span class="health-text">
                该设备自 <span class="highlight">{{ healthDiagnosis.installDate }}</span> 启用至今运行 <span class="highlight">{{ formatNumber(healthDiagnosis.runHours) }}h</span>，整体处于健康周期中段。主轴轴承经上次更换后运行稳定，温度趋势平稳；液压系统定位精度保持优秀。
              </span>
            </div>
            <div class="health-item warning" v-if="healthDiagnosis.hasRisk">
              <el-icon :size="14" color="var(--state-warning)">
                <Warning />
              </el-icon>
              <span class="health-text warning-text">
                <span class="font-semibold">风险提示</span>：刀库健康度降至 <span class="font-semibold">65</span>，接近预警阈值。基于近 6 月备件更换频率推算，建议在 <span class="font-mono">{{ healthDiagnosis.riskDays }}天内</span> 安排刀爪组件检查或更换，避免非计划停机。
              </span>
            </div>
            <div class="health-item success">
              ...
              <span class="health-text muted-text">
                <span class="success-label">维保建议</span>：下次保养重点关注刀库凸轮箱润滑及换刀机构间隙，同步检查主轴冷却回路。预计剩余寿命约 <span class="highlight">{{ healthDiagnosis.remainingMonths }}个月</span>。
              </span>
            </div>
          </div>
        </div>
      ...
      <div class="right-sidebar">
        ...
        <div class="kpi-grid eq-fade-in-1">
          ...
          <div class="kpi-card surface-panel">
            ...
            <span class="kpi-value">{{ formatNumber(kpiData.runHours) }}<span class="kpi-unit">h</span></span>
          </div>
        ...
        <div class="panel surface-panel workorder-panel">
          <div class="panel-header">
            ...
            <span class="pending-badge font-mono">{{ pendingCount }}待处理</span>
          </div>
            ...
            <div class="stats-item">
              <div class="pie-chart-wrap" @mouseleave="hoveredTypeIndex = -1">
                <svg viewBox="0 0 100 100" class="pie-svg">
                  ...
                  <path d="M50,50 L50,15 A35,35 0 0,1 85.6,28.6 Z" fill="var(--eq-primary)" class="pie-slice pie-slice-1"
                        @mouseenter="hoveredTypeIndex = 0"
                        :style="{ transform: hoveredTypeIndex === 0 ? 'translate(2px, -2px)' : 'none' }"/>
                  <path d="M50,50 L85.6,28.6 A35,35 0 0,1 82,71.4 Z" fill="var(--state-success)" class="pie-slice pie-slice-2"
                        @mouseenter="hoveredTypeIndex = 1"
                        :style="{ transform: hoveredTypeIndex === 1 ? 'translate(2px, -2px)' : 'none' }"/>
                  <path d="M50,50 L82,71.4 A35,35 0 0,1 50,85 Z" fill="var(--state-purple)" class="pie-slice pie-slice-3"
                        @mouseenter="hoveredTypeIndex = 2"
                        :style="{ transform: hoveredTypeIndex === 2 ? 'translate(0px, 2px)' : 'none' }"/>
                  <path d="M50,50 L50,85 A35,35 0 0,1 26.1,77.5 Z" fill="var(--state-info)" class="pie-slice pie-slice-4"
                        @mouseenter="hoveredTypeIndex = 3"
                        :style="{ transform: hoveredTypeIndex === 3 ? 'translate(-2px, 2px)' : 'none' }"/>
            ...
            <div class="stats-item">
              <div class="pie-chart-wrap" @mouseleave="hoveredStatusIndex = -1">
                <svg viewBox="0 0 100 100" class="pie-svg">
                  ...
                  <path d="M50,50 L50,15 A35,35 0 1,1 26.1,77.5 Z" fill="var(--state-success)" class="pie-slice pie-slice-1"
                        @mouseenter="hoveredStatusIndex = 0"
                        :style="{ transform: hoveredStatusIndex === 0 ? 'translate(2px, -2px)' : 'none' }"/>
                  <path d="M50,50 L26.1,77.5 A35,35 0 0,1 18,42.4 Z" fill="var(--state-warning)" class="pie-slice pie-slice-2"
                        @mouseenter="hoveredStatusIndex = 1"
                        :style="{ transform: hoveredStatusIndex === 1 ? 'translate(-2px, 0px)' : 'none' }"/>
                  <path d="M50,50 L18,42.4 A35,35 0 0,1 28.7,23.5 Z" fill="var(--state-error)" class="pie-slice pie-slice-3"
                        @mouseenter="hoveredStatusIndex = 2"
                        :style="{ transform: hoveredStatusIndex === 2 ? 'translate(0px, -2px)' : 'none' }"/>
                  <circle cx="50" cy="50" r="18" fill="rgba(15, 23, 42, 0.9)"/>
                ...
                <div class="stats-title">
                  ...
                  <span class="stats-label">处理状态</span>
                </div>
              </div>
            </div>
          </div>
...
function loadEquipmentList() {
  listDashboardEquipment().then(res => {
    ...
    if (equipmentList.value.length > 0) {
      selectedDevice.value = equipmentList.value[0].equipmentId
      loadDashboardData()
    }
  })
}
