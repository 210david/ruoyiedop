    ...
    <div class="surface">
      ...
      <div class="table-wrap">
        <el-table ref="tableRef" border v-loading="loading" :data="list" @selection-change="handleSelectionChange" @header-dragend="onHeaderDragEnd" class="app-table">
          ...
          <el-table-column label="缺陷代码" prop="defectCode" :width="colWidth('defectCode', 130)" resizable show-overflow-tooltip />
          <el-table-column label="缺陷名称" prop="defectName" :width="colWidth('defectName', 200)" resizable show-overflow-tooltip />
          <el-table-column label="缺陷等级" prop="defectLevel" :width="colWidth('defectLevel', 110)" resizable align="center"><template #default="scope"><span class="badge" :class="defectBadgeClass(scope.row.defectLevel)"><span class="dot"></span>{{ defectLevelLabel(scope.row.defectLevel) }}</span></template></el-table-column>
          <el-table-column label="缺陷分类" prop="defectCategory" :width="colWidth('defectCategory', 120)" resizable align="center"><template #default="scope"><dict-tag :options="qms_defect_category" :value="scope.row.defectCategory" /></template></el-table-column>
          <el-table-column label="状态" prop="status" :width="colWidth('status', 90)" resizable align="center"><template #default="scope"><span class="badge" :class="scope.row.status === '0' ? 'green' : 'gray'"><span class="dot"></span>{{ scope.row.status === '0' ? '正常' : '停用' }}</span></template></el-table-column>
...
const data = reactive({
  form: {},
  queryParams: { pageNum: 1, pageSize: 10, defectCode: undefined, defectName: undefined, defectLevel: undefined, defectCategory: undefined, status: undefined, params: {} },
  rules: {
...
const activeFilterCount = computed(() => { let c = 0; if (queryParams.value.defectCode) c++; if (queryParams.value.defectName) c++; if (queryParams.value.defectLevel) c++; if (queryParams.value.defectCategory) c++; if (queryParams.value.status) c++; return c })
...
function resetQuery() { queryParams.value.defectCode = undefined; queryParams.value.defectName = undefined; queryParams.value.defectLevel = undefined; queryParams.value.defectCategory = undefined; queryParams.value.status = undefined; queryParams.value.params = {}; handleQuery() }
...
function handleExport() { proxy.download('qms/defect/export', { ...queryParams.value }, `defect_${new Date().getTime()}.xlsx`) }
...
.qms-defect-page .field label { font-size:14px; font-weight:500; color:var(--ink-700); }
