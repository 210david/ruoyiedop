    ...
    <div class="surface">
      ...
      <div class="table-wrap">
        <el-table ref="tableRef" border v-loading="loading" :data="list" @selection-change="handleSelectionChange" @header-dragend="onHeaderDragEnd" class="app-table">
          ...
          <el-table-column label="业务类型" prop="bizType" :width="colWidth('bizType', 130)" resizable align="center"><template #default="scope"><span class="badge" :class="bizTypeBadgeClass(scope.row.bizType)"><span class="dot"></span>{{ bizTypeLabel(scope.row.bizType) }}</span></template></el-table-column>
          <el-table-column label="业务编号" prop="bizNo" :width="colWidth('bizNo', 160)" resizable show-overflow-tooltip />
          <el-table-column label="签名用户" prop="nickName" :width="colWidth('nickName', 120)" resizable />
          <el-table-column label="签名含义" prop="sigMeaning" :width="colWidth('sigMeaning', 160)" resizable show-overflow-tooltip />
          <el-table-column label="签名结果" prop="sigResult" :width="colWidth('sigResult', 100)" resizable align="center"><template #default="scope"><span class="badge" :class="sigResultBadgeClass(scope.row.sigResult)"><span class="dot"></span>{{ sigResultLabel(scope.row.sigResult) }}</span></template></el-table-column>
          <el-table-column label="签名意见" prop="sigOpinion" :width="colWidth('sigOpinion', 200)" resizable show-overflow-tooltip />
          <el-table-column label="IP地址" prop="sigIp" :width="colWidth('sigIp', 130)" resizable />
          <el-table-column label="签名时间" prop="sigTime" :width="colWidth('sigTime', 160)" resizable align="center" />
...
const data = reactive({
  form: {},
  queryParams: { pageNum: 1, pageSize: 10, bizType: undefined, bizNo: undefined, userName: undefined, sigResult: undefined, params: {} }
})
const { queryParams, form } = toRefs(data)

const activeFilterCount = computed(() => { let c = 0; if (queryParams.value.bizType) c++; if (queryParams.value.bizNo) c++; if (queryParams.value.userName) c++; if (queryParams.value.sigResult) c++; return c })
...
function resetQuery() { queryParams.value.bizType = undefined; queryParams.value.bizNo = undefined; queryParams.value.userName = undefined; queryParams.value.sigResult = undefined; queryParams.value.params = {}; handleQuery() }
...
.qms-esig-page .field label { font-size:14px; font-weight:500; color:var(--ink-700); }
